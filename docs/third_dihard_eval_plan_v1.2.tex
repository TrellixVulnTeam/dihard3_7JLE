...
% Following ensures appendix titles are formatted correct:
% 
...
% See p 5 of the RT-09 eval plan
...
\begin{itemize}
    ...
    %   - actual max: 80
    %   - median: around 25
    %   - 75th percentile: around 50
    %   - leaderboard uses best performing submission by team on progress set (no distinction between primary/contrastive)
    % - OpenSAT 19
    %   - no discussion of submission limits
    %   - leaderbord displays MOST RECENT primary-system submission output
    % - VoxCeleb 2020
...
% TODO: Should we keep this with the workshop?
...
\subsection*{Sources}
    \begin{itemize}
        ...
        %
        \item {\it FISHER} \\
            % corresponding source = FISHER
            % Fisher Phase II remainder stats
            % 
            % N: 8,284
            % duration: 1380.41841 hours
            % mean: 599.89 sec
            % s.d.: 2.97 sec
            % min: 404.80 sec
            % median: 599.95 sec
            % max: 600 sec
            %
            % total number of speakers acros both phases: 14,172
